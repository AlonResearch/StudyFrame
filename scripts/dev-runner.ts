...
export const DEFAULT_STUDYFRAME_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".studyframe"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("STUDYFRAME_PORT_OFFSET"),
  devInstance: optionalStringConfig("STUDYFRAME_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid STUDYFRAME_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `STUDYFRAME_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric STUDYFRAME_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed STUDYFRAME_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_STUDYFRAME_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly studyFrameHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(studyFrameHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      STUDYFRAME_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.STUDYFRAME_PORT = String(serverPort);
      ...
      delete output.STUDYFRAME_MODE;
      delete output.STUDYFRAME_NO_BROWSER;
      delete output.STUDYFRAME_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.STUDYFRAME_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.STUDYFRAME_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.STUDYFRAME_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.STUDYFRAME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.STUDYFRAME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.STUDYFRAME_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.STUDYFRAME_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.STUDYFRAME_MODE = "web";
      delete output.STUDYFRAME_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read STUDYFRAME_PORT_OFFSET/STUDYFRAME_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      studyFrameHome: input.studyFrameHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.STUDYFRAME_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.STUDYFRAME_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  studyFrameHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all StudyFrame data (equivalent to STUDYFRAME_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("STUDYFRAME_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to STUDYFRAME_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDYFRAME_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to STUDYFRAME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDYFRAME_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to STUDYFRAME_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("STUDYFRAME_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to STUDYFRAME_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("STUDYFRAME_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to STUDYFRAME_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("STUDYFRAME_PORT")),
  ),
