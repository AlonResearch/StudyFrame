...
function assertDescendantPid(
  ...
  if (pid === process.pid) {
    return Effect.fail(toProcessDiagnosticsError("Refusing to signal the StudyFrame server process."));
  }
      ...
      return descendant
        ...
        : Effect.fail(
            toProcessDiagnosticsError(
              `Process ${pid} is not a live descendant of the StudyFrame server.`,
            ),
          );
    }),
  );
